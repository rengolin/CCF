...
namespace http
{
  class HTTP3Endpoint : public enclave::QUICEndpoint
  {
  protected:
    http::Parser& p;
    ...
    HTTP3Endpoint(
      ...
      QUICEndpoint(session_id, writer_factory, std::move(ctx)),
      p(p_)
    {}
  ...
  public:
    ...
    {
      reinterpret_cast<HTTP3Endpoint*>(msg->data.self.get())
    ...
    void recv_(const uint8_t* data_, size_t size_)
    {
      ...
      while (true)
      {
        ...
        {
          LOG_FAIL_FMT("Error parsing HTTP3 request");
          LOG_DEBUG_FMT("Error parsing HTTP3 request: {}", e.what());

          auto response = http::Response(HTTP3_STATUS_BAD_REQUEST);
          response.set_header(
            http::headers::CONTENT_TYPE, http::headervalues::contenttype::TEXT);
          auto body = fmt::format(
            "Unable to parse data as a HTTP3 request. Error details are "
            "below.\n\n{}",
            e.what());
  ...
  class HTTP3ServerEndpoint : public HTTP3Endpoint, public http::RequestProcessor
  {
  private:
  ...
  public:
    HTTP3ServerEndpoint(
      ...
      ringbuffer::AbstractWriterFactory& writer_factory,
      std::unique_ptr<tls::Context> ctx) :
      HTTP3Endpoint(request_parser, session_id, writer_factory, std::move(ctx)),
      request_parser(*this),
      rpc_map(rpc_map),
      ...
      std::vector<uint8_t>&& body) override
    {
      ...
      try
      {
        ...
        {
          send_raw(http::error(
            HTTP3_STATUS_INTERNAL_SERVER_ERROR,
  ...
  class HTTP3ClientEndpoint : public HTTP3Endpoint,
  ...
  public:
    HTTP3ClientEndpoint(
      ...
      ringbuffer::AbstractWriterFactory& writer_factory,
      std::unique_ptr<tls::Context> ctx) :
      HTTP3Endpoint(response_parser, session_id, writer_factory, std::move(ctx)),
    ...
    void send(std::vector<uint8_t>&&) override
    {
      throw std::logic_error(
        "send() should not be called directly on HTTP3Client");
    }
